...
const char * const BITCOIN_CONF_FILENAME = "people.conf";
const char * const BITCOIN_PID_FILENAME = "peopled.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "people" is a composite category enabling all People-related debug output
            if(ptrCategory->count(string("people"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PeopleCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PeopleCore
    // Mac: ~/Library/Application Support/PeopleCore
    // Unix: ~/.peoplecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PeopleCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PeopleCore";
#else
    // Unix
    return pathRet / ".peoplecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty people.conf if it does not excist
